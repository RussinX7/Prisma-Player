...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: original } = await supabase.from("videos").select("title,folder_id,object_path,mime_type,size_bytes,duration_seconds,storage_provider").eq("id", id).eq("user_id", userId).eq("status", "ready").maybeSingle();
  ...
  const { data, error } = await supabase.from("videos").insert({
    user_id: userId, title: `${original.title} (cópia)`.slice(0, 200), folder_id: original.folder_id,
