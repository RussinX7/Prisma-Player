...
export async function GET(request: Request) {
  ...
  let [{ data: checkout }, { data: subscription }] = await Promise.all([
    admin.from("billing_checkouts").select("id,user_id,plan_id,external_id,status,checkout_type,paid_at,receipt_url,plan:billing_plans(name,slug)").eq("id", checkoutId).eq("user_id", userId).maybeSingle(),
    ...
    try {
      ...
      if (activated) {
        const refreshed = await Promise.all([
          admin.from("billing_checkouts").select("id,user_id,plan_id,external_id,status,checkout_type,paid_at,receipt_url,plan:billing_plans(name,slug)").eq("id", checkoutId).eq("user_id", userId).single(),
          admin.from("subscriptions").select("status,billing_method,current_period_end,plan:billing_plans(name,slug)").eq("user_id", userId).maybeSingle(),
        ]);
