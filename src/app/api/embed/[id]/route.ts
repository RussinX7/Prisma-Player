...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: playerConfig } = await supabase.from("player_configs").select("video_id, config, allowed_domains, published").eq("id", id).eq("published", true).maybeSingle();
  ...
  const domains = Array.isArray(playerConfig.allowed_domains) ? playerConfig.allowed_domains : [];
  if (host && !domainAllowed(host, domains)) return NextResponse.json({ error: "domain_not_allowed" }, { status: 403 });
  ...
  const { data: signed, error } = await supabase.storage.from("videos").createSignedUrl(video.object_path, 3600);
  ...
  return NextResponse.json({ id, title: video.title, source: signed.signedUrl, type: video.mime_type, config }, { headers: { "cache-control": "private, max-age=300" } });
}
