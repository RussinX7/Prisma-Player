...
export default async function BillingPage() {
  ...
  return (
    <main className="dashboard-content pb-16 space-y-6">
        ...
        {subscription?.status === "active" ? (
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-bold text-[#191A23]">{plan?.name ?? "Plano Prisma Pro"}</h2>
            ...
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-1.5 text-xs font-medium text-slate-700">
          ...
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-[#191A23]">Acesso completo liberado</h2>
            <p className="text-xs font-medium text-slate-500 mt-0.5">
          ...
          <div>
            <h2 className="text-xl font-bold text-[#191A23]">Escolha o plano ideal para sua operação</h2>
      ...
      <div>
        <h2 className="text-xl font-bold text-[#191A23]">Planos Prisma Player</h2>
        <p className="mt-0.5 text-xs font-medium text-slate-500">
        ...
        <div className="mt-4 grid items-stretch gap-5 xl:grid-cols-3">
          {(plansResult.data as BillingPlan[] ?? []).map((item) => {
            ...
            return (
              <article
                key={item.id}
                className={`relative flex min-h-full flex-col rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xs ${
                  item.is_featured ? "ring-2 ring-[#B9FF66]" : ""
                }`}
              >
                {item.is_featured && (
                  <span className="absolute right-5 top-5 rounded-full bg-[#B9FF66] border border-black/5 px-3 py-0.5 text-[10px] font-bold text-[#191A23]">
                    Mais Escolhido
                  </span>
                )}
                <p className="pr-20 text-lg font-bold text-[#191A23]">{item.name}</p>
                <p className="mt-1 min-h-10 text-xs font-medium text-slate-500 leading-relaxed">{item.description}</p>
                <p className="mt-3 text-3xl font-bold text-[#191A23] tracking-tight">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2.5">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                    ...
                    <strong className="mt-0.5 block text-xs font-bold text-[#191A23]">{item.included_plays.toLocaleString("pt-BR")}/mês</strong>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-2.5">
                    <p className="flex items-center gap-1 text-[10px] font-semibold uppercase text-slate-400">
                      Storage <OverageInfo text={`Após o limite: ${formatStorageOverage(item.storage_overage_cents_per_gb)} por GB/mês.`} />
                    </p>
                    <strong className="mt-0.5 block text-xs font-bold text-[#191A23]">{item.storage_gb.toLocaleString("pt-BR")} GB</strong>
                  </div>
                </div>
                ...
                <div className="mt-5 flex-1 space-y-2.5">
                  {getPlanBenefits(item)
                    ...
                    .map((feature) => (
                      <p key={feature} className="flex items-start gap-2 text-xs font-medium text-slate-700">
                ...
                {isCurrentPlan ? (
                  <div className="mt-5 rounded-xl bg-emerald-50 border border-emerald-200 p-2.5 text-center text-xs font-bold text-emerald-800">
                    Seu plano atual
                  </div>
                ) : (
                  <div className="mt-5">
      ...
      <div className="pt-4">
        <div>
          <h2 className="flex items-center gap-2.5 text-xl font-bold text-[#191A23]">
            <Image src="/prisma-credits.png" width={36} height={36} alt="Créditos Prisma" className="h-8 w-8 object-contain" />
            Créditos Prisma IA
          </h2>
          <p className="mt-0.5 text-xs font-medium text-slate-500">
            Saldo atual: <strong className="font-bold text-[#191A23]">{walletResult.data?.balance ?? 0} créditos</strong>. Os créditos não expiram.
          </p>
        </div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {(creditsResult.data ?? []).map((item) => (
            <article
              key={item.id}
              className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs space-y-2.5"
            >
              ...
              <p className="pr-14 text-sm font-bold text-[#191A23]">{item.name}</p>
              <p className="text-2xl font-bold text-[#191A23]">
                {item.credits + item.bonus_credits} <span className="text-xs font-normal text-slate-400">créditos</span>
              </p>
              {item.bonus_credits > 0 && (
                <p className="text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-0.5 w-fit">
                  +{item.bonus_credits} de bônus incluídos
                </p>
              )}
              <p className="text-xl font-bold text-[#191A23]">{formatBRL(item.amount_cents)}</p>
...
function OverageInfo({ text }: { text: string }) {
  return (
    <span className="group relative inline-flex">
      <button type="button" aria-label={text} className="grid size-4 place-items-center rounded-full text-slate-400 outline-none">
        <Info size={12} />
      </button>
      <span role="tooltip" className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 hidden w-52 -translate-x-1/2 rounded-xl border border-slate-200 bg-slate-900 p-2.5 text-[11px] font-medium leading-relaxed text-white shadow-lg group-hover:block group-focus-within:block">
        {text}
      </span>
    </span>
  );
