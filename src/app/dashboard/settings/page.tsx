...
import { ArrowLeft, Bell, CheckCircle2, CircleUserRound, CreditCard, Database, HardDrive, KeyRound, Lock, Play, Shield, Smartphone, Sparkles, Trash2, User, UserPlus, Users } from "lucide-react";
...
function OverviewPanel({ overview, profile }: { overview: Overview | null; profile: Profile | null }) {
  ...
  return <div className="space-y-5">
    <section className="overflow-hidden rounded-[18px] border themeable-bg-canvas themeable-border-hairline"><div className="flex flex-wrap items-start justify-between gap-4 border-b p-6 themeable-border-hairline"><div><p className="text-[12px] font-semibold uppercase tracking-[.14em] text-prisma-blue">Sua operação</p><h2 className="mt-2 text-[25px] font-semibold tracking-[-.4px] themeable-text-ink">{profile?.full_name || "Conta Prisma"}</h2><p className="mt-1 text-[13px] themeable-text-ink-muted-48">{plan ? `${plan.name} ativo` : overview.access.trialStatus === "active" ? "Teste gratuito ativo" : "Escolha um plano para publicar"}</p></div><Link href="/dashboard/billing" className="rounded-full bg-prisma-blue px-4 py-2.5 text-[13px] font-semibold text-white">Gerenciar plano</Link></div>
...
function PasswordPanel({ setMessage }: { setMessage: (value: string) => void }) { const [currentPassword, setCurrentPassword] = useState(""), [password, setPassword] = useState(""), [confirm, setConfirm] = useState(""); async function update() { if (password !== confirm) return setMessage("As senhas não coincidem."); const response = await fetch("/api/account/password", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ currentPassword, password }) }); if (response.ok) { setMessage("Senha atualizada com sucesso."); setCurrentPassword(""); setPassword(""); setConfirm(""); return; } const data = await response.json().catch(() => null) as { error?: string } | null; setMessage(PASSWORD_ERROR_MESSAGES[data?.error ?? ""] ?? "Use pelo menos 10 caracteres, com letras e números."); } return <Card title="Alterar senha" description="Crie uma senha exclusiva para manter sua conta protegida."><div className="grid gap-5 sm:grid-cols-2"><label className="text-[13px] font-medium themeable-text-ink sm:col-span-2">Senha atual<Input type="password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} autoComplete="current-password" /></label><label className="text-[13px] font-medium themeable-text-ink">Nova senha<Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" /></label><label className="text-[13px] font-medium themeable-text-ink">Confirmar senha<Input type="password" value={confirm} onChange={(event) => setConfirm(event.target.value)} autoComplete="new-password" /></label></div><div className="mt-4 flex flex-wrap gap-4 text-[12px] themeable-text-ink-muted-48"><span className="flex gap-1"><CheckCircle2 size={15} />10 caracteres</span><span className="flex gap-1"><CheckCircle2 size={15} />Letras e números</span></div><button onClick={update} disabled={!currentPassword || !password || !confirm} className="mt-5 min-h-11 rounded-full bg-prisma-blue px-5 text-[14px] text-white disabled:opacity-40">Atualizar senha</button></Card>; }
...
function SecurityPanelFixed({ setMessage, adminMfaRequired }: { setMessage: (value: string) => void; adminMfaRequired: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <Card title="Sessão e privacidade"><div className="space-y-3"><Info icon={KeyRound} title="Sessão protegida" detail="Suas credenciais permanecem protegidas durante a navegação" /><Info icon={Shield} title="Conta isolada" detail="Seus vídeos e métricas não ficam acessíveis a outras contas" /></div></Card>
  </div>;
}
