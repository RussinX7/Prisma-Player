  ...
  return <>
    ...
    <section className="dashboard-content flex flex-1 flex-col">
      ...
      <div className="mt-5 min-h-[360px] flex-1 rounded-[18px] border p-4 themeable-bg-canvas themeable-border-hairline sm:p-6">
        {visibleVideos.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{visibleVideos.map((item) => <button key={item.id} type="button" onClick={() => edit(item)} className="overflow-hidden rounded-[14px] border text-left themeable-border-hairline"><div className="aspect-video bg-black">{item.signed_url && <video src={item.signed_url} preload="metadata" muted className="h-full w-full object-contain" />}</div><div className="p-4"><h3 className="truncate font-semibold themeable-text-ink">{item.title}</h3><p className="mt-1 text-[12px] capitalize themeable-text-ink-muted-48">{item.status === "ready" ? "Publicado" : item.status}</p></div></button>)}</div> : <EmptyState title="Nenhum vídeo encontrado" description={selectedFolder ? "Esta pasta ainda não tem vídeos. Faça um upload para adicioná-lo diretamente aqui." : "Seus vídeos salvos aparecerão aqui."} actionLabel="Adicionar vídeo" onAction={() => setImportOpen(true)} />}
      </div>
    </section>
