...
interface StudioConfig {
  ...
  smartProgress: boolean; progressColor: string; progressHeight: number; smartAutoplay: boolean; autoplayMessage: string; autoplayTextColor: string; autoplayBackground: string; autoplayRadius: number; playbackRate: number;
  headlineEnabled: boolean; headline: string; headlineColor: string; headlineBackground: string; headlineSize: number; headlineAlign: "left" | "center" | "right"; trafficEnabled: boolean; domains: string[]; browserLanguage: string; allowedCountries: string; allowedDevices: string[]; blockVpn: boolean; urlKeyEnabled: boolean; accessToken: boolean;
  ...
  loop: boolean; muted: boolean; smartPause: boolean; fullscreenDesktop: boolean; fullscreenMobile: boolean; videoDuration: number; aspectRatio: number;
...
const initialConfig: StudioConfig = {
  ...
  loop: false, muted: false, smartPause: true, fullscreenDesktop: true, fullscreenMobile: true, videoDuration: 0, aspectRatio: 16 / 9,
  assets: {},
};
...
export default function VslStudio() {
  ...
  const playerStyle = { "--player-accent": config.smartProgress ? config.progressColor : config.accent, "--player-progress-height": `${config.progressHeight}px`, borderRadius: `${config.radius}px`, background: config.colorStyle === "gradient" ? `linear-gradient(135deg, ${config.background}, ${config.gradientEnd})` : config.background } as CSSProperties;
  ...
  return <div className="min-h-dvh bg-[#f5f5f7] text-[#1d1d1f] dark:bg-[#1d1d1f] dark:text-white">
    ...
    <div className="grid min-h-[calc(100dvh-64px)] lg:grid-cols-[320px_minmax(0,1fr)]">
      ...
      <main className="flex min-w-0 flex-col bg-white p-4 text-[#1d1d1f] dark:bg-black dark:text-white sm:p-6 lg:p-8">
        ...
        <div className="flex flex-1 items-center justify-center overflow-hidden bg-transparent p-4 sm:p-8">
          <div className="w-full">
            {config.headlineEnabled && <h2 className="mx-auto mb-4 max-w-2xl px-4 py-3 font-semibold leading-tight" style={{ color: config.headlineColor, backgroundColor: config.headlineBackground, fontSize: `${config.headlineSize}px`, textAlign: config.headlineAlign, borderRadius: `${Math.min(config.radius, 16)}px` }}>{config.headline}</h2>}
