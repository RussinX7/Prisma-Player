...
interface StudioConfig {
  ...
  loop: boolean; muted: boolean; smartPause: boolean; fullscreenDesktop: boolean; fullscreenMobile: boolean; videoDuration: number;
...
const initialConfig: StudioConfig = {
  ...
  loop: false, muted: false, smartPause: true, fullscreenDesktop: true, fullscreenMobile: true, videoDuration: 0,
  assets: {},
};
...
export default function VslStudio() {
  ...
  function handleMetadata(metadata: { duration: number; width: number; height: number }) {
    ...
    setConfig((current) => ({ ...current, videoDuration: metadata.duration, ctaStart: Math.min(current.ctaStart, Math.max(0, Math.floor(metadata.duration))) }));
      ...
      <main className="flex min-w-0 flex-col bg-white p-4 text-[#1d1d1f] dark:bg-black dark:text-white sm:p-6 lg:p-8">
        ...
        <div className="flex flex-1 items-center justify-center overflow-hidden bg-transparent p-4 sm:p-8">
          <div className="w-full">
            {config.headlineEnabled && <h2 className="mx-auto mb-4 max-w-2xl text-center text-[clamp(18px,2.6vw,30px)] font-semibold leading-tight">{config.headline}</h2>}
...
function renderPanel(module: ModuleId, c: StudioConfig, u: <K extends keyof StudioConfig>(key: K, value: StudioConfig[K]) => void, onPoster: (file: File, kind: "start" | "pause" | "end") => void, onCaption: (file: File) => void) {
  ...
  if (module === "traffic") return <><TextInput label="Adicionar domínio" placeholder="checkout.exemplo.com" onEnter={(value) => { const clean = value.replace(/^https?:\/\//, "").replace(/\/.*$/, ""); if (clean && !c.domains.includes(clean)) u("domains", [...c.domains, clean]); }} />{c.domains.map((domain) => <div key={domain} className="flex min-h-11 items-center gap-2 rounded-[11px] bg-black/5 px-3 text-[13px] dark:bg-white/5"><Globe2 size={15} /><span className="flex-1 truncate">{domain}</span><button onClick={() => u("domains", c.domains.filter((item) => item !== domain))}>×</button></div>)}<div className="rounded-[12px] border border-prisma-blue/20 bg-prisma-blue/5 p-3 text-[12px] leading-relaxed themeable-text-ink-muted-48">Sem domínios cadastrados, o player funciona em qualquer site. A Prisma registra sinais de automação e tráfego suspeito no Analytics sem bloquear visitantes legítimos.</div></>;
  if (module === "actions") return <><TextInput label="Texto do botão" value={c.ctaText} onChange={(v) => u("ctaText", v)} /><TextInput label="Link" value={c.ctaUrl} onChange={(v) => u("ctaUrl", v)} /><TimeRange label="Aparecer em" value={c.ctaStart} max={Math.max(1, Math.floor(c.videoDuration || 1))} onChange={(v) => u("ctaStart", v)} /><p className="panel-help">Duração real do vídeo: {formatPlayerTime(c.videoDuration)}. O CTA só pode aparecer dentro desse período.</p></>;
