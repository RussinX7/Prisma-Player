...
export default function VslStudio() {
  ...
  async function save(): Promise<string | undefined> {
    ...
    if (video?.id) {
      ...
      const payload = await response.json().catch(() => null) as { playerConfig?: { id?: string }; error?: string; fields?: string[] } | null;
      if (!response.ok) {
        setSaveError(payload?.error === "invalid_pixel_config" ? "Revise os IDs dos pixels ativos. O formato informado não é válido." : "Não foi possível salvar as alterações.");
    ...
    <div className="grid h-[calc(100dvh-64px)] min-h-0 grid-cols-1 grid-rows-[58px_minmax(0,1fr)_150px] overflow-hidden lg:grid-cols-[220px_minmax(0,1fr)_340px] lg:grid-rows-[minmax(0,1fr)_180px]">
      ...
      <aside className="order-3 hidden min-h-0 overflow-hidden border-l border-black/10 bg-white dark:border-white/10 dark:bg-[#272729] lg:block">
        <div className="h-full overflow-y-auto overscroll-contain p-4">
          <ModulePanel module={active ?? "style"} config={config} update={update} onPoster={handlePoster} onCaption={handleCaption} playerId={playerId} videoId={video?.id} />
        </div>
      </aside>
      ...
      {active && <aside className="fixed inset-x-0 bottom-0 top-16 z-50 overflow-hidden bg-white/98 backdrop-blur-xl dark:bg-[#1d1d1f]/98 lg:hidden">
        ...
        <div className="h-[calc(100%-56px)] overflow-y-auto overscroll-contain p-4"><ModulePanel module={active} config={config} update={update} onPoster={handlePoster} onCaption={handleCaption} playerId={playerId} videoId={video?.id} /></div>
      </aside>}
