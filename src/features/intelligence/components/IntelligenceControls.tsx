...
import { AlertTriangle, ArrowLeft, BarChart3, BellRing, Check, FileClock, LoaderCircle, Radio, Send, Users, Webhook } from "lucide-react";
...
const tools = [
  { id: "reports", icon: FileClock, title: "Relatórios automáticos", description: "Resumo real da operação no inbox e por e-mail quando o SMTP estiver configurado.", capability: "automatic_reports" },
...
export default function IntelligenceControls({ capabilities, benchmarkData, videos }: { capabilities: Capabilities; videoCount: number; benchmarkData: BenchmarkData; videos: VideoOption[] }) {
  ...
  if (active) return <section className="mt-6 rounded-[24px] border bg-white p-5 themeable-border-hairline dark:bg-[#1d1d1f] sm:p-7">
    <button onClick={() => { setActive(null); setNotice(""); }} className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-prisma-blue"><ArrowLeft size={16} /> Voltar para Inteligência</button>
