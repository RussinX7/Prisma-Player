...
export default function AccessGate({ children }: { children: React.ReactNode }) {
  ...
  const [state, setState] = useState<{ checkedPath: string | null; access: boolean; trial: boolean }>({ checkedPath: null, access: false, trial: false });
  useEffect(() => {
    if (unrestricted) return;
    let active = true;
    fetch("/api/account/access", { cache: "no-store" })
      .then((r) => r.json())
      .then((data) => { if (active) setState({ checkedPath: pathname, access: Boolean(data.hasAccess), trial: data.trialStatus === "available" }); })
      .catch(() => { if (active) setState({ checkedPath: pathname, access: false, trial: false }); });
    return () => { active = false; };
  }, [pathname, unrestricted]);
  if (unrestricted) return children;
  if (state.checkedPath !== pathname) return <div className="flex min-h-[70vh] items-center justify-center"><LoaderCircle className="animate-spin text-prisma-blue" /></div>;
