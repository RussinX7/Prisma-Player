...
export default function CheckoutResult({ checkoutId }: { checkoutId: string }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      attempts += 1;
      if (active && attempts < 20 && response.ok) window.setTimeout(refresh, 3000);
    }
    void refresh();
    return () => { active = false; };
  }, [checkoutId]);
