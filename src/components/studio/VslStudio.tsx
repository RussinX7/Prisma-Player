    ...
    <div className="grid min-h-[calc(100dvh-64px)] lg:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="border-b border-black/10 bg-white dark:border-white/10 dark:bg-[#272729] lg:border-b-0 lg:border-r">
        <div className="sticky top-16 max-h-[calc(100dvh-64px)] overflow-y-auto p-3 sm:p-4">
          {active ? <ModulePanel module={active} config={config} update={update} onBack={() => setActive(null)} onPoster={(file) => { if (posterUrl) URL.revokeObjectURL(posterUrl); setPosterUrl(URL.createObjectURL(file)); setPosterPreviewActive(true); }} onCaption={(file) => { if (captionTrack) URL.revokeObjectURL(captionTrack.src); setCaptionTrack({ src: URL.createObjectURL(file), kind: "subtitles", label: file.name, srclang: "pt-BR", default: true }); }} /> : <>
